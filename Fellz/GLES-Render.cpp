...
// Box2DDebugDraw class

Box2DDebugDraw::Box2DDebugDraw(b2World* world)
{
	this->world = world;
	
	debugDrawning = new GLESDebugDraw(PTM_RATIO);
	world->SetDebugDraw(debugDrawning);
	debugDrawning->SetFlags(b2Draw::e_shapeBit | b2Draw::e_jointBit);
}

Box2DDebugDraw* Box2DDebugDraw::create(b2World* world)
{
	Box2DDebugDraw * pRet = new Box2DDebugDraw(world);
	pRet->autorelease();
	return pRet;
}
